...
parser.add_argument('--exp_name', type=str, default='officeHome/UDA/abla1-woNRC', help='experiment name')
parser.add_argument('--source_dir', type=str, default='save/officeHome/UDA/CE-T0.05/Product', metavar='B',help='path to source list')
parser.add_argument('--source_num', type=int, default=15, help="num of source classes")
parser.add_argument('--source_path', type=str, default='../data/officeHome/source_Product.txt', metavar='B',help='path to source list')

parser.add_argument('--target_path', nargs='+', default=['../data/officeHome/target_Clipart.txt'],help='list of target paths')
parser.add_argument("--gpu_devices", type=int, nargs='+', default=[2], help="")
...
imagenet_folder = os.path.join('../data/imagenet_validation')
imagenet_train = torchvision.datasets.ImageFolder(imagenet_folder,transform=train_transforms)
imagenet_train_loader = torch.utils.data.DataLoader(imagenet_train,batch_size=batch_size,shuffle=True,drop_last=False,num_workers=4)
imagenet_test = torchvision.datasets.ImageFolder(imagenet_folder,transform=test_transforms)
imagenet_test_loader = torch.utils.data.DataLoader(imagenet_test,batch_size=batch_size,shuffle=False,drop_last=False,num_workers=4)
...
for target_path in args.target_path:
    ...
    acc_init = 0
    max_iters = 60 * len(target_loader)
    interval_iter = max_iters // 60
    iter_num = 0
    ...
    imagenet_train_iter = iter(imagenet_train_loader)
    ...
    while iter_num < max_iters:
        ...
        try:
            img_3rd, label_3rd,index_3rd = next(imagenet_train_iter)
        except:
            imagenet_train_iter = iter(imagenet_train_loader)
            img_3rd, label_3rd,index_3rd = next(imagenet_train_iter)
        ...
        img_3rd = img_3rd.cuda()
        outputs_source = source_model(img_3rd)
        preds_source = nn.Softmax(dim=1)(outputs_source)
        feat_3rd = feat_extractor(img_3rd)
        outputs_3rd = classifier(feat_3rd)
        probs_3rd = nn.Softmax(dim=1)(outputs_3rd)
        ...
        distill_loss = -torch.sum(preds_source*torch.log(probs_3rd+1e-5),dim=1).mean()
        ...
        loss_nrc_ = 0.0 #loss_N + loss_E + loss_D
        loss_nrc = (1-alpha) * loss_nrc_
